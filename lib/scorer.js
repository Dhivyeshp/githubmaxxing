...
function scoreCommitConsistency(activity) {
  const { daysSinceLastCommit } = activity;
  if (daysSinceLastCommit === null) return 0;
  if (daysSinceLastCommit <= 7) return 100;
  if (daysSinceLastCommit <= 30) return 75;
  if (daysSinceLastCommit <= 90) return 40;
  if (daysSinceLastCommit <= 180) return 20;
  return 0;
}
...
export function computeScores(githubData) {
  ...
  const categories = {
    ...
    commits: scoreCommitConsistency(activity),
